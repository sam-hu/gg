...
`make install` copies the source into an owned temporary directory, runs the locked build there with an isolated npm home/cache/config, packages it, and installs only the runtime package under `~/.local/share/gg`. The user-facing `~/.local/bin/gg` is created only after the staged executable passes `--help` validation. An exclusive ownership-marked lock serializes install operations, and signal/failure handlers remove owned staging trees.

`make uninstall` verifies both the installation manifest and executable symlink before deleting anything. It removes the dedicated prefix, link, stale ownership-marked staging directories, lock, and installer-created directories only when they are empty. Pre-existing directories and unrelated executables are never removed.
