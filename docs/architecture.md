...
When preview detects a conflict, `gg` checks that the worktree is clean, checks out the affected branch, and invokes a normal `git rebase --onto`. Git owns the index and conflict files. `gg continue` advances that rebase and resumes the saved queue. A rebase completed directly with `git rebase --continue` is also accepted: the next stack-changing `gg` command verifies that the branch reflog moved directly from the journaled old head, records the resulting head and parent revision, resumes the saved queue, and then starts the requested operation. `gg abort --force` calls `git rebase --abort`, restores captured refs and metadata, and returns to the original branch. Recovery refuses to overwrite a ref or topology changed outside the recorded operation, including an externally deleted branch.
...
`make install` copies the current source into an owned temporary directory, runs the locked build there with an isolated npm home/cache/config, packages it, and installs only the runtime package under `~/.local/share/gg`. It also installs identical, digest-tracked copies of the `gg-stacked-branches` skill in the Codex user skill root at `~/.agents/skills` and the Claude Code user skill root at `~/.claude/skills`. Every run replaces an existing managed installation so local development updates take effect. The replacement is staged and validated before the previous package is swapped out; failures restore the previous package and skill copies. The user-facing `~/.local/bin/gg` is created only after the staged executable passes `--help` validation. An exclusive ownership-marked lock serializes install operations, and signal/failure handlers remove owned staging trees.

`make uninstall` verifies the installation manifest, executable symlink, and skill digests before deleting anything. It removes the dedicated prefix, link, unchanged owned skill copies, stale ownership-marked staging directories, lock, and installer-created directories only when they are empty. Pre-existing directories and modified or unrelated skills are retained. Installation intentionally replaces an existing file or symlink only at the exact `~/.local/bin/gg` executable path; an unrecognized package directory or skill is never replaced.
